...
$arResult["PROPERTY_LIST"][0] = 'NAME';
$arResult["PROPERTY_LIST"][1] = '33';
$arResult["PROPERTY_LIST"][2] = 'PREVIEW_TEXT';
?>
...
<div id="formPopup">
		...
		<?endif?>
			<form id="add_question" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data"  name="iblock_add">
				<img src="/img/pic15c.gif" alt="" class="formRemover1" />
				
				<?if(!isset($arResult["MESSAGE"]) or $arResult["MESSAGE"]==''):?>
								...
								<ul>
									...
									<?if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"] > 0)):?>
										...
										<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
												...
												<li><span><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>*<?endif?><?if($propertyID!=38):?></span><?endif?> <?//CHAK ?>
													...
													switch ($INPUT_TYPE):
														...
														case "T":
															...
															?>
													<textarea class="w136" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
															<?
															}
														break;
														...
														case "N":
															...
															?>
															<input class="w237" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>" /><?
															...
															switch ($type):
																...
																case "radio":
																	...
																	foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum)
																	{
																		...
																		?>
														<input type="<?='checkbox'//$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /></span>
										...
										<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
												...
												<li><span><?=GetMessage("IBLOCK_FORM_CAPTCHA_TITLE")?></span>
													<input type="hidden" name="captcha_sid" value="<?=$arResult["CAPTCHA_CODE"]?>" />
													<img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" />
												</li>
										
												<li><span><?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?>*:<span>
												<input class="w237" type="text" name="captcha_word" maxlength="50" value=""></li>
