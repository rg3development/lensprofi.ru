...
if(isset($arResult["ID"]))
{
	...
	if($arParams["SET_TITLE"])
		// комментарий аркадия для помены заголовка
		if (!empty($arResult['PROPERTY_51'])) {
			$APPLICATION->SetTitle($arResult['PROPERTY_51'], $arTitleOptions);
		} else {
	...
	if(isset($arResult["PROPERTIES"][$arParams["BROWSER_TITLE"]]))
	{
		...
		// комментарий аркадия для помены заголовка
		if (!empty($arResult['PROPERTY_51'])) {
			$APPLICATION->SetPageProperty("title", $arResult['PROPERTY_51']);
		}
	...
	{
		// комментарий аркадия для помены заголовка
		if (!empty($arResult['PROPERTIES']['TITLE_PRODUCT']['VALUE'])) {
			$APPLICATION->SetPageProperty("title", $arResult['PROPERTIES']['TITLE_PRODUCT']['VALUE']);
		} else {
