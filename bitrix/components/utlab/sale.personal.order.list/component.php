...
if ($ID > 0 && $_REQUEST["COPY_ORDER"] == "Y")
{
	...
	//CHAK parse basket id
	$ARR_BASKET_ID = array();
	if(isset($_REQUEST['ids']))
	{
		$arr_ids = explode('x', $_REQUEST['ids']);
		foreach($arr_ids as $one_id)
		{
			if($one_id != '') $ARR_BASKET_ID[] = $one_id;
		}
	};
	
	// echo '<pre>';
	// print_R($ARR_BASKET_ID);
	// echo '</pre>';
	// die();
	...
	if ($arOrder = $dbOrder->Fetch())
	{
		...
		{
			//CHAK add only nesessary items
			if(count($ARR_BASKET_ID)>0 and !in_array($arBasket["ID"], $ARR_BASKET_ID)) continue;
