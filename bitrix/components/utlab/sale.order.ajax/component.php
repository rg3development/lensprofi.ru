...
$arParams["DELIVERY_NO_AJAX"] = (($arParams["DELIVERY_NO_AJAX"] == "N") ? "N" : "Y");
...
$arResult = Array(
		...
		"WEIGHT_UNIT" => htmlspecialchars(COption::GetOptionString('sale', 'weight_unit', false, SITE_ID)),
...
if (!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N")
{
	...
	{
		if (in_array($vname, Array("USER_LOGIN", "USER_PASSWORD", "do_authorize", "NEW_NAME", "NEW_LAST_NAME", "NEW_EMAIL", "NEW_GENERATE", "NEW_LOGIN", "NEW_PASSWORD", "NEW_PASSWORD_CONFIRM", "captcha_sid", "captcha_word", "do_register", "AJAX_CALL")))
			continue;
		if(is_array($vvalue))
		{
			foreach($vvalue as $k => $v)
				$arResult["POST"][htmlspecialchars($vname."[".$k."]")] = htmlspecialchars($v);
		}
		else
...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y")
{
	if(IntVal($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		// !!! CHAK - вот тут сейвим выборку из корзины, пока они не проапдейчена добавлением в заказ
		$HACK_ITEMS = array();
		...
		while ($arBasketItems = $dbBasketItems->GetNext())
		{
			...
			по аналогии с корзиной - убираем код, который "восстанавливает" каталожные цены
			...
			if (strlen($arBasketItems["CALLBACK_FUNC"])>0)
			{
				...
				$dbBasketItemsTmp = CSaleBasket::GetList(Array(), Array("ID" => $arBasketItems["ID"]), false, false, $arSelFields);
		...
		// !!!! chak
		$HACK_order_price = $arResult["ORDER_PRICE"];
					...
					if ($bErrorField)
						{
							...
							// CHAK, added for showing errors
							$arResult['ARR_ERROR_PROP'][$arOrderProps["ID"]] = $arOrderProps["ID"];
						};
				}
			}
		...
		while($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, Array(&$arResult, &$arUserResult));
		/* Person Type End */
		...
		if ($arUserResult["TAX_LOCATION"] > 0 || $arResult["bUsingVat"] == "Y")
		{
			...
			{
				if(DoubleVal($arResult["DELIVERY_PRICE"])>0 && $arParams["COUNT_DELIVERY_TAX"] == "Y")
					$arResult["VAT_SUM"] += roundEx($arResult["DELIVERY_PRICE"] * $arResult["VAT_RATE"] / (1 + $arResult["VAT_RATE"]));
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					...
					/* chak here was exploding mail by @ for user login. we cancel it
					...
					*/
					...
					//chak - тут добавим все свои поля
					$NEW_NAME = $arUserResult["ORDER_PROP"][2]; // checked
					$NEW_PHONE = $arUserResult["ORDER_PROP"][3]; //checked
					$NEW_PHONE_CODE = $arUserResult["ORDER_PROP"][5]; // checked
					//additional - сохраним поля уведомления
					$UF_EMAIL_REMINDER = '';
					$UF_SMS_REMINDER = '';
					$UF_DATE_REMIND = array();
					
					$UF_DATE_REMIND[0] = $arUserResult["ORDER_PROP"][11];
					if($arUserResult["ORDER_PROP"][12]=='Y') $UF_SMS_REMINDER=1;
					if($arUserResult["ORDER_PROP"][13]=='Y') $UF_EMAIL_REMINDER=1;
					...
					$arAuthResult = $user->Add(Array(
						...
						"LID" => SITE_ID,
						//CHAK
						'PERSONAL_PHONE' => $NEW_PHONE,
						'UF_PHONE_CODE' => $NEW_PHONE_CODE,
						'UF_SMS_REMINDER' => $UF_SMS_REMINDER,
						'UF_DATE_REMIND' => $UF_DATE_REMIND,
						'UF_EMAIL_REMINDER' => $UF_EMAIL_REMINDER,
						)
						);
			...
			// END OF --- AUTOREGISTERING
			//CHAK добавить код для сохранения напоминания зарегистрированному пользователю
			if($USER->IsAuthorized())
			{
				$uid = $USER->GetID();
		...
		if($USER->IsAuthorized() && empty($arResult["ERROR"]))
		{
			...
			if (empty($arResult["ERROR"]))
			{
				...
				//CHAK!!!
				$CHAK_PROFILE_NAME = $arUserResult["ORDER_PROP"][9];
				...
				while ($arOrderProperties = $dbOrderProperties->Fetch())
				{
					...
					if (strlen($curVal) > 0)
					{
						...
						if ( $arOrderProperties["USER_PROPS"] == "Y" && IntVal($arUserResult["PROFILE_ID"])<=0 && IntVal($arUserResult["PROFILE_ID_new"])<=0)
						{
							...
							// CHAK!!!! тут мы переименуем профиль
							if($CHAK_PROFILE_NAME!='') 
							{
								$arUserResult["PROFILE_NAME"] =  $CHAK_PROFILE_NAME;
								if(mb_strlen($CHAK_PROFILE_NAME)>20)
								{
									$arUserResult["PROFILE_NAME"] = mb_substr($CHAK_PROFILE_NAME, 0, 20).'...';
								};
							}
							//----
							$arFields = array(
			...
			if (empty($arResult["ERROR"]))
			{
				...
				// !!!! CHAK возьем для мыла сохраненную корзину
				$dbBasketItems = $HACK_dbBasketItems;
				
				//CHAK - добавим "цена за упаковку"
				//while ($arBasketItems = $dbBasketItems->Fetch())
				foreach($HACK_ITEMS as $arBasketItems)
				{
					$strOrderList .= $arBasketItems["NAME"]." - ".$arBasketItems["QUANTITY"]." ".GetMessage("SOA_SHT").", цена за упаковку: ".SaleFormatCurrency($arBasketItems["PRICE"], $arBasketItems["CURRENCY"]);
					$strOrderList .= "\n";
				}
				
				// формула взята из 1497 строки
				$CHAK_FULL_PRICE = $HACK_order_price + $arResult["DELIVERY_PRICE"] + $arResult["TAX_PRICE"] - $arResult["DISCOUNT_PRICE"];
				...
				$arFields = Array(
					...
					"PRICE" => SaleFormatCurrency( $CHAK_FULL_PRICE, $arResult["BASE_LANG_CURRENCY"]),
