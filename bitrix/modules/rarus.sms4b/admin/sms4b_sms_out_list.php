...
//multy and single actions processing
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$row->AddViewField("GUID", $f_GUID);
	$row->AddViewField("SENDERNAME", $f_SenderName);
	$row->AddViewField("DESTINATION", $f_Destination);
	$row->AddViewField("STARTSEND", $f_StartSend);
	$row->AddViewField("LASTMODIFIED", $f_LastModified);
	$row->AddViewField("COUNTPART", $f_CountPart);
	$row->AddViewField("SENDPART", $f_SendPart);
	$row->AddViewField("CODETYPE", $f_CodeType);
	$row->AddViewField("TEXTMESSAGE", str_replace("
	",'<br>',$f_TextMessage));
	$row->AddViewField("SALE_ORDER", $f_Sale_Order);
	$row->AddViewField("POSTING", $f_Posting);
	$row->AddViewField("EVENTS", $f_Events);
endwhile;
