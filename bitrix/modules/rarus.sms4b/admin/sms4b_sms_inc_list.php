...
while($arRes = $rsData->NavNext(true, "f_")):
	$row =& $lAdmin->AddRow($f_GUID, $arRes); 
	$row->AddViewField("GUID", $f_GUID);
	$row->AddViewField("MOMENT", $f_Moment);
	$row->AddViewField("TIMEOFF", $f_TimeOff);
	$row->AddViewField("SOURCE", $f_Source);
	$row->AddViewField("DESTINATION", $f_Destination);
	
	if (intval($f_Coding == 0))
	{
		$row->AddViewField("BODY", htmlspecialchars(str_replace("
",'<br>',$SMS4B->decode($f_Body, $f_Coding))));
	}
	else
	{
