...
************************************/
	protected function getbodyrec($funcname='',$param=array(),$nameclient)
	{
		...
		{
			//если функция SaveMessages, то в ней параметр "list" формируется нестандартно, поэтому
		//параметры для нее формируем следующим образом ....
...
';
		...
		if ($response != -1)
		{
			switch ($funcname)
			{
				...
				case "CheckUser":
						...
						{
							$this->LastError = 'Доступ запрещен';
				...
				case "ChangePassword":
						...
						{
							$this->LastError = 'Ошибка смены пароля';
							$this->user_check = false;
							return false;
						}
				break;
				...
				case "ChangeUserPassword":
						...
						{
							$this->LastError = 'Ошибка смены пользовательского пароля';
							return false;
						}
				break;
	...
	**************************/
	...
	protected function LoadIn($xml)
	{
		...
		//результат не является СМС-кой (т.е новые смс не загружены) 
		...
		switch ($event_name)
		{
				...
				if ($b_send == "Y")
				{
					...
					if(preg_match("/^([\+\-\(\)0-9]+?)@phone.sms$/i",$params["EMAIL"],$find))
					{
						...
						if ($phone_num = $SMS4B->is_phone($phone_num))
						{
							...
							global $APPLICATION;
							$params["EMAIL"] = '';
							$APPLICATION->throwException("SMS с кодом подверждения отправлено");
							return false;
						}
					}
				}
			...
			case "TICKET_CHANGE_FOR_TECHSUPPORT":
				...
				if ($b_send == "Y")
				{
					...
					//если нету ответственного, то отсылаем уведомление администратору или сотруднику тп с имейлом по умолчанию
	...
	public function AddIBlock(&$arFields)
	{
		...
		//получаем ID элемента, который добавляется
		...
		{ 
			if(COption::GetOptionInt('intranet', 'iblock_calendar') == $ari["IBLOCK_ID"])
			{
				if($ar_res_sec["CREATED_BY"] <> $USER->GetID())
				{
					...
					{
						$text = "Новое событие с вашим участием - ".$ari["NAME"].". "."
".$ari["DETAIL_TEXT"]; 
		...
		if($b_update == "Y")
		{
			if(COption::GetOptionInt('intranet', 'iblock_calendar') == $arFields["IBLOCK_ID"])
			{
				 if($arFields["CREATED_BY"] <> $USER->GetID())
				 {
					 ...
					 if($phone_num = $SMS4B->is_phone($arUser["PERSONAL_MOBILE"]))
					 {			 	
						$text = "Изменения в событии - ".$arFields["NAME"]." от ".$arFields["ACTIVE_FROM"].". "."
".$arFields["DETAIL_TEXT"]; 
	...
	public function EventsPosting($arFields)
	{
		...
		global $SMS4B;
		//добавить условие на  символьный номер
	...
	public function SendSmsPack($message, $to, $sender='', $startUp_p='', $dateActual_p='', $period_p = '')
	{
		...
		//избавляемся от двойников, заменяем восьмерки
		...
		foreach($to as $arInd)
		{
			...
			$one_sms = array(
						...
						"StartUp"		=> $startUp,	/*время отложенной отправки*/
						"Period"		=> $period,		/*период суток, когда отправка разрешена*/
						"Destination"	=> $arInd,		/*адрес получателя*/
						"Source"		=> $sender,		/*адрес источника*//*"1C-WebSMS-R"*/
						"Body"			=> $body,		/*текст сообщения*/
						"Encoded" 		=> $encoded,	/*тип кодировки*/
						"dton" 			=> $dton,		/*тип номера получателя*/
						"dnpi" 			=> $dton,		/*numeric plan indicator*/
						"ston" 			=> $ston,		/*тип номера отправителя*/
						"snpi" 			=> $snpi,		/*numeric plan indicator*/
						"TimeOff" 		=> $dateActual,	/*время актуальности сообщения*/
						"Priority"		=> 0,			/*приоритет*/
						"NoRequest"		=> $request		/*отчет о доставке*/
				);
		...
		else
		{
			...
			foreach($big_array as $arIndex)
			{
				...
				//собираем результирующую информацию об "успешности" отправки
