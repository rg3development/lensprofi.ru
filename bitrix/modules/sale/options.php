...
//-->
	...
	<tr>
		<td colspan="2">
			...
			while($arPersonType = $dbPersonType -> GetNext())
			{
				$aTabs1[] = Array("DIV"=>"oedit".$arPersonType["ID"], "TAB" => $arPersonType["NAME"]." (".htmlspecialchars(implode(", ", $arPersonType["LIDS"])).")", "TITLE" => $arPersonType["NAME"]." (".htmlspecialchars(implode(", ", $arPersonType["LIDS"])).")");
			...
			foreach($personType as $val)
			{
				...
				<script language="JavaScript">
				<!--
				ActionFileChange(<?=CUtil::JSEscape($val["ID"])?>, '');
				//-->
				</script>
				<?
			}
