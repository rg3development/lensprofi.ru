...
while ($arSite = $dbSite->Fetch())
{
	...
	$WEIGHT_UNIT[$arSite["LID"]] = htmlspecialchars(COption::GetOptionString('sale', 'weight_unit', "", $arSite["LID"]));
	$WEIGHT_KOEF[$arSite["LID"]] = htmlspecialchars(COption::GetOptionString('sale', 'weight_koef', 1, $arSite["LID"]));
}
		...
		<td>
			<select name="filter_person_type[]" multiple size="3">
				...
				while ($personType = $l->Fetch()):
					?><option value="<?echo htmlspecialchars($personType["ID"])?>"<?if (is_array($filter_person_type) && in_array($personType["ID"], $filter_person_type)) echo " selected"?>>[<?echo htmlspecialchars($personType["ID"]) ?>] <?echo htmlspecialchars($personType["NAME"])?> <?echo "(".htmlspecialchars(implode(", ", $personType["LIDS"])).")";?></option><?
				endwhile;
				?>
			</select>
		</td>
