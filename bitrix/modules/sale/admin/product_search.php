...
if (!$bBadBlock)
{
	...
	while ($arItems = $dbResultList->NavNext(true, "f_"))
	{
		...
		$arParams = "{'id' : '".$arItems["ID"]."', 'name' : '".CUtil::JSEscape($arItems["NAME"])."', 'url' : '".CUtil::JSEscape($URL)."', 'price' : '".CUtil::JSEscape($currentPrice)."', 'currency' : '".CUtil::JSEscape($arPrice["PRICE"]["CURRENCY"])."', 'weight' : '".DoubleVal($arCatalogProduct["WEIGHT"])."', 'priceType' : '".CUtil::JSEscape($arPrice["PRICE"]["CATALOG_GROUP_NAME"])."', 'catalogXmlID' : '".CUtil::JSEscape($arIBlock["XML_ID"])."', 'productXmlID' : '".CUtil::JSEscape($f_XML_ID)."', 'callback' : 'CatalogBasketCallback',	'orderCallback' : 'CatalogBasketOrderCallback', 'cancelCallback' : 'CatalogBasketCancelCallback', 'payCallback' : 'CatalogPayOrderCallback'}";
