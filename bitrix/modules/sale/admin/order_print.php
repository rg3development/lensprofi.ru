...
if ($REQUEST_METHOD == "POST" && strlen($Print)>0 && check_bitrix_sessid() && $bUserCanViewOrder)
{
	if(count($REPORT_ID) > 0)
	{
		...
		{
			?>
			window.open('sale_print.php?doc=<?echo CUtil::JSEscape($REPORT_ID[$i]) ?>&ORDER_ID=<?echo $ID ?>&BASKET_IDS=<?echo urlencode($sBasket) ?>&QUANTITIES=<?echo urlencode($sQuantity) ?>', '_blank');
			<?
		}
		?>
		</script>
