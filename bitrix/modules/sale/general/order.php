...
class CAllSaleOrder
{
	...
	function PayOrder($ID, $val, $bWithdraw = True, $bPay = True, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		if ($val == "Y")
		{
			...
			$bSend = true;
			$db_events = GetModuleEvents("sale", "OnOrderPaySendEmail");
			while ($arEvent = $db_events->Fetch())
	...
	function DeliverOrder($ID, $val, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		if ($val == "Y")
		{
			...
			$bSend = true;
			$db_events = GetModuleEvents("sale", "OnOrderDeliverSendEmail");
			while ($arEvent = $db_events->Fetch())
				if (ExecuteModuleEventEx($arEvent, Array($ID, &$eventName, &$arFields))===false)
					$bSend = false;
	...
	function CancelOrder($ID, $val, $description = "")
	{
		...
		if ($val == "Y")
		{
			...
			$bSend = true;
			$db_events = GetModuleEvents("sale", "OnOrderCancelSendEmail");
			while ($arEvent = $db_events->Fetch())
				if (ExecuteModuleEventEx($arEvent, Array($ID, &$eventName, &$arFields))===false)
					$bSend = false;
	...
	function StatusOrder($ID, $val)
	{
		...
		$bSend = true;
		$db_events = GetModuleEvents("sale", "OnOrderStatusSendEmail");
		while ($arEvent = $db_events->Fetch())
			...
			if(!empty($arSites))
			{
				...
				while($arOrder = $dbOrder -> GetNext())
				{
					...
					if(in_array($date_insert, $arReminder[$arOrder["LID"]]["days"]))
					{
						...
						$bSend = true;
						$db_events = GetModuleEvents("sale", "OnOrderRemindSendEmail");
						while ($arEvent = $db_events->Fetch())
