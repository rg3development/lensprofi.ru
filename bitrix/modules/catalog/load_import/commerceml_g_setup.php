	...
	<tr>
		...
		<td align="left" nowrap>
			<select name="IBLOCK_TYPE_ID">
				...
				<?
				$rsIBlockTypes = CIBlockType::GetList(array($by=>$order));
				while ($arIBlockType = $rsIBlockTypes->Fetch())
				{
					if($arIBLang = CIBlockType::GetByIDLang($arIBlockType["ID"], LANGUAGE_ID))
   					{
   						?><option value="<? echo $arIBlockType['ID']; ?>"<? echo ($arIBlockType['ID'] == $IBLOCK_TYPE_ID ? ' selected' : ''); ?>><? echo htmlspecialchars($arIBLang["NAME"]); ?> [<? echo $arIBlockType['ID']; ?>]</option><?
					}
				}
				?>
			</select>
