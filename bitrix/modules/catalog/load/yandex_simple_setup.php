	...
	if (CModule::IncludeModule("iblock"))
	{
		...
		<table width="100%">
			<tr>
				...
				<td valign="top">
					...
					<table width="100%" cellspacing="1" cellpadding="2" border="0">
						...
						<?
						$db_res = CIBlock::GetList(Array("iblock_type"=>"asc", "name"=>"asc"),array('CHECK_PERMISSIONS' => 'Y','MIN_PERMISSION' => 'R'));
						while ($res = $db_res->Fetch())
						{
							if ($ar_res1 = CCatalog::GetByID($res["ID"]))
							{
								...
								while($arSite = $rsSites->Fetch())
									$strSiteList .= htmlspecialchars($arSite["SITE_ID"]).' ';
								$strSiteList = substr($strSiteList,0,-1).')';
								?>
								<tr>
									<td class="tablebody1">
										<font class="tablebodytext">
										<?echo "[".$res["IBLOCK_TYPE_ID"]."] ".htmlspecialchars($res["NAME"])." ".$strSiteList;?>
										</font>
									</td>
			...
			<tr>
				...
				<td width="100%" valign="top">
					<font class="text">
					<?echo GetMessage("CET_SAVE_FILENAME");?> <input type="text" name="SETUP_FILE_NAME" value="<?echo htmlspecialchars(strlen($SETUP_FILE_NAME)>0 ? $SETUP_FILE_NAME : (COption::GetOptionString("catalog", "export_default_path", "/upload/"))."yandex_".mt_rand(0, 999999).".php"); ?>" size="50">
					</font>
					<br><br>
				</td>
			</tr>
