...
if (strlen($strExportErrorMessage)<=0)
{
	...
	if (strlen($strExportErrorMessage)<=0)
	{
		...
		for ($i = 0; $i < count($field_code); $i++)
		{
			...
			elseif (substr($field_code[$i], 0, strlen("IP_PROP"))=="IP_PROP" && $field_needed[$i]=="Y")
			{
				$strTempo = substr($field_code[$i], strlen("IP_PROP"));
				$selectProps[] = $strTempo;
				$selectArray[] = "PROPERTY_".$strTempo;
			}
		...
		if (false == empty($selectProps))
		{
			$rsProps = CIBlockProperty::GetList(array(),array('IBLOCK_ID' => $IBLOCK_ID,'PROPERTY_TYPE' => 'F'));
			while ($arProp = $rsProps->Fetch())
			{
				if (in_array($arProp['ID'],$selectProps))
