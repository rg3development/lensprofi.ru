...
if ($STEP>2)
{
	...
	{
		//$strCSVError .= GetMessage("CATI_TOO_MANY_TABLES").'  '.$tableLinksCount."<br>";
...
<table border="0" cellspacing="1" cellpadding="0" width="99%">
	<tr>
		...
		<td align="right">
			...
			<?if ($STEP>1):?>
			<input type="submit" class="button" name="backButton"
...
?>
				...
				<table id="table_r" border="0" cellspacing="0" cellpadding="3" width="100%">
					...
					<tr>
						...
						<td valign="top" align="left" width="50%" class="tablebody">
							<font class="text">
							<input type="checkbox" name="first_line_names" value="Y" <?if ($first_line_names=="Y" || (!isset($first_line_names) && strlen($strCSVError)<=0)) echo "checked"?>>
							</font>
						</td>
					</tr>
				</table>
			...
			?><table width="100%" border="0" cellspacing="2" cellpadding="3">
				<tr>
					<td valign="top" align="left" class="tablehead">
						<font class="tableheadtext"><input type="checkbox" name="field_needed_all" id="field_needed_all" value="Y" onclick="checkAll(this,<? echo $intCountAvailFields; ?>);"<? echo ($intCountChecked == $intCountAvailFields ? ' checked' : ''); ?>>&nbsp;
				...
				for ($i = 0; $i < $intCountAvailFields; $i++)
				{
					...
					<tr>
						<td valign="top" class="tablebody">
							...
							<input type="checkbox" name="field_needed[<?echo $i ?>]" id="field_needed_<? echo $i; ?>"<?/*if ($field_needed[$i]=="Y" || (!isset($field_needed) && strlen($strCSVError)<=0)) echo "checked"; */ if (in_array($i,$arCheckID)) echo "checked";?> value="Y" onclick="checkOne(this,<? echo $intCountAvailFields; ?>);">
							</font>
						</td>
						<td valign="top" class="tablebody">
							<font class="tablefieldtext">
						...
						<td valign="top" align="center" class="tablebody">
							...
							<input type="hidden" name="field_code[<?echo $i ?>]" value="<?echo htmlspecialchars($arAvailFields[$i]["value"]) ?>">
	...
	<tr>
		...
		<td valign="top" align="left" nowrap class="tablebody">
			<font class="tablebodytext">
			<input type="text" class="typeinput" name="SETUP_FILE_NAME" size="40" value="<?echo htmlspecialchars(strlen($SETUP_FILE_NAME)>0? $SETUP_FILE_NAME :(COption::GetOptionString("catalog", "export_default_path", "/upload/"))."export_file_".mt_rand(0, 999999).".csv"); ?>"><br>
