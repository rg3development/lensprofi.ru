...
function CatalogBasketCallback($productID, $quantity = 0, $renewal = "N")
{
	...
	if (empty($arPrice["PRICE"]["CATALOG_GROUP_NAME"]))
	{
		...
		{
			$rsCatGroups = CCatalogGroup::GetList(array(),array('ID' => $arPrice["PRICE"]["CATALOG_GROUP_ID"]),false,false,array('NAME','NAME_LANG'));
...
function CatalogBasketOrderCallback($productID, $quantity, $renewal = "N")
{
	...
	if(!empty($arPrice["DISCOUNT"]))
	{
		...
		if ($arCoupon = $dbCoupon->Fetch())
		{
			...
			$arFieldsCoupon = Array("DATE_APPLY" => Date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL", SITE_ID))));
...
function CatalogRecurringCallback($productID, $userID)
{
	...
	if (empty($arPrice))
	{
		...
		if ($nearestQuantity = CCatalogProduct::GetNearestQuantityPrice($productID, 1, $USER->GetUserGroupArray()))
		{
			...
			$arPrice = CCatalogProduct::GetOptimalPrice($productID, $quantity, $USER->GetUserGroupArray(), "Y");
		}
	}
...
/*		$arCatalog = CCatalog::GetByID($arIBlock['ID']);
		...
		{
			$rsSKUProps = CIBlockElement::GetProperty($arIBlock['ID'],$arProduct['ID'],array(),array('ID' => $arCatalog['SKU_PROPERTY_ID']));
			if ($arSKUProp = $rsSKUProps->Fetch())
			{
				if (0 < intval($arSKUProp['VALUE']))
				{
					...
					{
						$arProduct["DETAIL_PAGE_URL"] = $arParent['DETAIL_PAGE_URL'];
					}
				}
			}
		} */
