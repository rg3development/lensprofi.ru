...
class CAllCatalog
{
	function CheckFields($ACTION, &$arFields, $ID = 0)
	{
		...
		if (true == $boolResult)
		{
			if ('ADD' == $ACTION)
			{
				if (false == is_set($arFields,'IBLOCK_ID'))
				{
				...
				{
					$rsIBlocks = CIBlock::GetByID(array(),array('ID' => $arFields['IBLOCK_ID']));
		...
		if (true == $boolResult)
		{
			if ('ADD' == $ACTION)
			{
				if (false == is_set($arFields, "PRODUCT_IBLOCK_ID"))
				{
				...
				elseif (0 < $arFields["PRODUCT_IBLOCK_ID"])
				{
					$rsIBlocks = CIBlock::GetByID(array(),array('ID' => $arFields['PRODUCT_IBLOCK_ID']));
					if (!($arIBlock = $rsIBlocks->Fetch()))
					{
				...
				if (false == is_set($arFields, "SKU_PROPERTY_ID"))
				{
			...
			{
				$boolLocalFlag = ((true == is_set($arFields,'PRODUCT_IBLOCK_ID')) == (true == is_set($arFields,'SKU_PROPERTY_ID')));
				if (false == $boolLocalFlag)
				{
	...
	function Update($ID, $arFields)
	{
		...
		if ('' != trim($strUpdate))
		{
			...
			if (true == is_set($GLOBALS["CATALOG_CATALOG_CACHE"],$ID))
...
/*		$ID = intval($ID);
		...
		$rsProducts = CCatalog::GetList(array(),array('PRODUCT_IBLOCK_ID' => $ID),false,false,array('IBLOCK_ID','PRODUCT_IBLOCK_ID','SKU_PROPERTY_ID'));
	...
	{
/*		$ID = intval($ID);
		$rsProducts = CCatalog::GetList(array(),array('SKU_PROPERTY_ID' => $ID),false,false,array('IBLOCK_ID','PRODUCT_IBLOCK_ID','SKU_PROPERTY_ID'));
		return $rsProducts->Fetch(); */
	...
	function OnBeforeIBlockElementDelete($ID)
	{
		...
		if (0 < $ID)
		{
			...
			if ($arItem = $rsItems->Fetch())
			{
				...
				if ((true == is_array($arCatalog)) && (true == in_array($arCatalog['CATALOG_TYPE'],array('P','X'))))
				{
...
/*		$dbRes = CIBlockElement::GetList(array(), array("IBLOCK_ID" => $ID));
		while ($arRes = $dbRes->Fetch())
		{
			if (!CCatalogProduct::Delete($arRes["ID"]))
			{
				$arMsg[] = array('id' => 'CATALOG_PRODUCT','text' => GetMessage('BT_MOD_CATALOG_ERR_CANNOT_DELETE_PRODUCTS_INFO'));
				$obError = new CAdminException($arMsg);
				$APPLICATION->ThrowException($obError);
				return false;
			}
	...
	function GetByIDExt($ID)
	{
		...
		if (false != $mxPRResult)
		{
			...
			{
				//$mxSKU = CCatalog::GetSkuInfoByProductID($ID);
	...
	function UnLinkSKUIBlock($ID)
	{
		...
		if (true == $boolResult)
		{
			...
			if ($arCatalog = $rsCatalog->Fetch())
			{
				...
				if (true == is_set($GLOBALS["CATALOG_CATALOG_CACHE"],$arCatalog['IBLOCK_ID']))
					unset($GLOBALS["CATALOG_CATALOG_CACHE"][$arCatalog['IBLOCK_ID']]);

/*				$rsProps = CIBlockProperty::GetByID($arCatalog['SKU_PROPERTY_ID'],$arCatalog['IBLOCK_ID']);
				if ($arProp = $rsProps->Fetch())
				{
					$ibp = new CIBlockProperty();
					$arFields = $arProp;
					unset($arFields['ID']);
					$arFields['IS_REQUIRED'] = 'N';
	...
	function LinkSKUIBlock($ID,$SKUID)
	{
		...
		if ((false === $arSKUProp) || ((true == is_array($arSKUProp)) && ('N' != $arSKUProp['MULTIPLE'])))
		{
			$arOFProperty = array(
