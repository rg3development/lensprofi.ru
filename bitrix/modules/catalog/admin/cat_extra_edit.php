...
if ($REQUEST_METHOD=="POST" && strlen($Update) > 0 && !$bReadOnly /*$catalogModulePermissions>="W"*/ && check_bitrix_sessid())
{
	...
	{
		if (empty($apply))
			LocalRedirect("/bitrix/admin/cat_extra.php?lang=".LANGUAGE_ID/*.GetFilterParams("filter_", false)*/);
		else
			LocalRedirect("/bitrix/admin/cat_extra_edit.php?lang=".LANGUAGE_ID."&ID=".$ID /*.GetFilterParams("filter_", false)*/);
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/bitrix/admin/cat_extra.php?lang=".LANGUAGE_ID /*.GetFilterParams("filter_", false)*/
	)
);
...
if ($ID > 0 && !$bReadOnly /*$catalogModulePermissions >= "W"*/)
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/bitrix/admin/cat_extra_edit.php?lang=".LANGUAGE_ID /*.GetFilterParams("filter_", false)*/
		);
...
?>
	...
	<tr>
		...
		<td width="60%">
			<input type="text" name="NAME" size="50" value="<? echo htmlspecialchars($str_NAME); ?>">
		</td>
	</tr>
	<tr>
...
$tabControl->Buttons(
		array(
				...
				"disabled" => $bReadOnly,
				"back_url" => "/bitrix/admin/cat_extra.php?lang=".LANGUAGE_ID /*.GetFilterParams("filter_", false)*/
			)
	);
?>
