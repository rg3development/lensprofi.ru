...
if (!$bReadOnly && $REQUEST_METHOD=="POST" && strlen($Update)>0 && /*$catalogPermissions=="W" &&*/ check_bitrix_sessid())
{
	...
	if ($PRODUCT_IDS_CNT >= 0)
	{
		...
		{
			$intValue = intval(${"PRODUCT_IDS_".$i});
			if ($intValue > 0)
			{
				$arProductID[] = $intValue;
			}
		}
	}
	...
	{
		for ($i = 0, $cnt = count($SECTION_IDS); $i < $cnt; $i++)
		{
			$intValue = intval($SECTION_IDS[$i]);
			if ($intValue > 0)
			{
				$arSectionID[] = $intValue;
			}
		}
	}
	...
	{
		for ($i = 0, $cnt = count($GROUP_IDS); $i < $cnt; $i++)
		{
			$intValue = intval($GROUP_IDS[$i]);
			if ($intValue > 0)
			{
				$arGroupID[] = $intValue;
			}
	...
	{
		for ($i = 0, $cnt = count($CAT_IDS); $i < $cnt; $i++)
		{
			$intValue = intval($CAT_IDS[$i]);
			if ($intValue > 0)
			{
	...
	$arIBlockID = array();
	if (isset($IBLOCK_IDS) && is_array($IBLOCK_IDS))
	{
		for ($i = 0, $cnt = count($IBLOCK_IDS); $i < $cnt; $i++)
		{
			$intValue = intval($IBLOCK_IDS[$i]);
			if ($intValue > 0)
			{
				$arIBlockID[] = $intValue;
			}
	...
	*/
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		else
		{
			...
			{
				LocalRedirect("/bitrix/admin/cat_discount_edit.php?lang=".LANGUAGE_ID."&ID=".$ID.GetFilterParams("filter_", false));
			}
		}
	}
	else
	...
	<tr>
		...
		<td>
			<select name="VALUE_TYPE">
				...
				<option value="S"<?if ($str_VALUE_TYPE == "S") echo " selected";?>><?= GetMessage("DSC_TYPE_SALE") ?></option>
			</select>
		</td>
	</tr>
	<tr>
...
?>
	...
	<tr>
		...
		<td valign="top" width="60%">
			...
			<tr id="ID_TR_COUPON1">
				<td>
					<input type="text" id="COUPON" name="COUPON" size="30" maxlength="30" value="<?= ($bOneCoupon) ? htmlspecialchars($COUPON) : "" ?>">
					&nbsp;&nbsp;&nbsp;
...
/*						var oCoupon = document.fdiscount_edit.COUPON;
			...
			<tr id="ID_TR_COUPON2">
				<td>
					...
					{
						?>
						<a href="/bitrix/admin/cat_discount_coupon.php?lang=<?= LANGUAGE_ID ?>&set_filter=Y&filter_discount_id=<?= $ID ?>"><?= GetMessage("CDEN_DCPN_LIST_ALT") ?></a><br>
						<?
					}
					?>
	...
	<tr>
		<td valign="top" width="40%"><? echo GetMessage("DSC_IBLOCKS"); ?></td>
		<td valign="top" width="60%">
			<?
			$LOCAL_IBLOCK_CACHE = array();
			$LOCAL_IBLOCKTYPE_CACHE = array();
			$rsIBlockType = CIBlockType::GetList(array("sort" => "asc","id"=>"asc"), array("ACTIVE"=>"Y"));
			while($arr = $rsIBlockType->Fetch())
			{
				if($ar = CIBlockType::GetByIDLang($arr["ID"], LANGUAGE_ID, true))
				{
					$LOCAL_IBLOCKTYPE_CACHE[$arr["ID"]] = $ar["NAME"];
				}
				$rsIBlocks = CIBlock::GetList(array("iblock_type" => "asc","sort" => "asc"),array("=TYPE" => $arr['ID']));
				while ($arIBlock = $rsIBlocks->Fetch())
				{
					$LOCAL_IBLOCK_CACHE[$arIBlock['ID']] = "[".$LOCAL_IBLOCKTYPE_CACHE[$arIBlock["IBLOCK_TYPE_ID"]]."] ".$arIBlock["NAME"];
				}
			}

			$arDiscountIBlockList = array();
			if ($bVarsFromForm)
			{
				if (isset($IBLOCK_IDS) && is_array($IBLOCK_IDS))
				{
			...
			else
			{
				if ($ID > 0)
				{
					$dbDiscountIBlockList = CCatalogDiscount::GetDiscountIBlocksList(array(), array("DISCOUNT_ID" => $ID));
					while ($arDiscountIBlock = $dbDiscountIBlockList->Fetch())
					{
						$arDiscountIBlockList[] = intval($arDiscountIBlock["IBLOCK_ID"]);
					}
				}
			}
			?>
			<select name="IBLOCK_IDS[]" multiple="multiple" size="5">
			<?
			foreach ($LOCAL_IBLOCK_CACHE as $intIBlockID => $strIBlockName)
			{
				?><option value="<? echo $intIBlockID; ?>"<? echo (in_array($intIBlockID,$arDiscountIBlockList) ? ' selected' : ''); ?>><? echo htmlspecialchars($strIBlockName); ?></option><?
			}
			?>
			</select>
		</td>
	...
	<tr>
		...
		<td valign="top" width="60%">
			...
			<tr><td>
				...
				for ($i = 0, $cnt = count($arDiscountSections); $i < $cnt; $i++)
				{
					...
					if (!isset($LOCAL_IBLOCK_CACHE[intval($arSection["IBLOCK_ID"])]))
					{
						...
						if ($arIBlock = $dbIBlock->Fetch())
						{
							...
							{
								if ($arIBlockType = CIBlockType::GetByIDLang($arIBlock["IBLOCK_TYPE_ID"], LANGUAGE_ID, true))
			...
			<tr>
				<td align="center" nowrap>
					...
					<tr><td>
						<?
						if (!empty($SECTION_IBLOCK_ID))
							$SECTION_IBLOCK_ID = intval($SECTION_IBLOCK_ID);
						else
							$SECTION_IBLOCK_ID = 0;
