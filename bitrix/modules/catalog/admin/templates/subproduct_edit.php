...
function checkSubForm(e)
{
	...
	if (BX('SUBCAT_ROW_COUNTER').value > 0 && !BX('subprice_useextform').checked)
	{
...
function SetSubFieldsStyle(table_id)
{
	...
	for(var i=0; i<n; i++)
		...
		$aTabs1[] = array("DIV" => "subcat_edit5", "TAB" => GetMessage("C2IT_DISCOUNTS"), "TITLE" => GetMessage("C2IT_DISCOUNTS_D"));
...
<input type="checkbox" name="subprice_useextform" id="subprice_useextform" value="Y" onclick="toggleSubPriceType()" <?=$bUseExtendedPrice ? 'checked="checked"' : ''?> /><label for="subprice_useextform">&nbsp;<?=GetMessage('C2IT_PRICES_USEEXT')?></label>
<br />
...
?>
	...
	<tr>
		...
		<td><input type="checkbox" name="SUBCAT_VAT_INCLUDED" id="SUBCAT_VAT_INCLUDED" value="Y" <?=$str_CAT_VAT_INCLUDED == 'Y' ? 'checked="checked"' : ''?> <?=$bReadOnly ? 'disabled readonly' : ''?> /><label for="SUBCAT_VAT_INCLUDED"><?=GetMessage('CAT_YES')?></label></td>
	</tr>
</table>
<script type="text/javascript">
...
<div id="subprices_simple" style="display: <?=$bUseExtendedPrice ? 'none' : 'block'?>;">
		...
		?>
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="subcatalog_price_table">
			<tr>
				<td width="40%">
					<?
					$arBaseGroup = CCatalogGroup::GetBaseGroup();
					$arBasePrice = CPrice::GetBasePrice($PRODUCT_ID, $arPriceBoundaries[0]["FROM"], $arPriceBoundaries[0]["TO"]);
					echo GetMessage("BASE_PRICE")?> (<? echo GetMessage('C2IT_PRICE_TYPE'); ?> "<? echo htmlspecialchars(!empty($arBaseGroup['NAME_LANG']) ? $arBaseGroup['NAME_LANG'] : $arBaseGroup["NAME"]); ?>"):
				</td>
				<td width="60%">
					<script type="text/javascript">
					...
					?>
					<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="SUBCAT_BASE_PRICE" name="SUBCAT_BASE_PRICE" value="<?echo htmlspecialchars($str_CAT_BASE_PRICE) ?>" size="30" OnBlur="ChangeSubBasePrice(this)">
				</td>
			</tr>
			<tr>
				<td>
...
<script type="text/javascript">
SetSubFieldsStyle('subcatalog_price_table');
</script>
		<?
		$bFirst = true;
		...
		<?
			...
			<tr <?if ($bReadOnly) echo "disabled" ?>>
				<td valign="top" align="left">
					<? echo htmlspecialchars(!empty($arCatalogGroup["NAME_LANG"]) ? $arCatalogGroup["NAME_LANG"] : $arCatalogGroup["NAME"]); ?>
					<?if ($arPrice):?>
				...
				<td valign="top" align="center">
					<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="SUBCAT_PRICE_<?echo $arCatalogGroup["ID"] ?>" name="SUBCAT_PRICE_<?echo $arCatalogGroup["ID"] ?>" value="<?echo htmlspecialchars($str_CAT_PRICE) ?>" size="8" OnChange="ChangeSubPrice(<?= $arCatalogGroup["ID"] ?>)">
				</td>
				<td valign="top" align="center">
					<?
...
<div id="subprices_ext" style="display: <?=$bUseExtendedPrice ? 'block' : 'none'?>;">
		...
		{
			//addSubNewElementsGroup('subcontainer3', 'submodel3', 'SUBCAT_ROW_COUNTER', false, 'select');
			CloneSubBasePriceGroup();

			var i, n;
			for (i = 0; i < arCatalogGroups.length; i++)
			{
				//addSubNewElementsGroup('subcontainer3_'+arCatalogGroups[i], 'submodel3_'+arCatalogGroups[i], 'SUBCAT_ROW_COUNTER_'+arCatalogGroups[i], false, 'select');
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
					...
					<table border="0" cellspacing="1" cellpadding="3" id="SUBBASE_PRICE_GROUP_TABLE">
						...
						<tbody id="subcontainer3">
							...
							for ($i = 0; $i < $intCount; $i++)
							{
								...
								<tr id="submodel3">
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> name="SUBCAT_BASE_QUANTITY_FROM_<?= $ind ?>" value="<?echo ($str_CAT_BASE_QUANTITY_FROM != 0 ? htmlspecialchars($str_CAT_BASE_QUANTITY_FROM) : "") ?>" size="3" OnChange="ChangeSubBaseQuantityEx(this)">
										<input type="hidden" name="SUBCAT_BASE_ID[<?= $ind ?>]" value="<?= htmlspecialchars($str_CAT_BASE_ID) ?>">
									</td>
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> name="SUBCAT_BASE_QUANTITY_TO_<?= $ind ?>" value="<?echo ($str_CAT_BASE_QUANTITY_TO != 0 ? htmlspecialchars($str_CAT_BASE_QUANTITY_TO) : "") ?>" size="3" OnChange="ChangeSubBaseQuantityEx(this)">
									</td>
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="SUBCAT_BASE_PRICE_<?= $ind ?>" name="SUBCAT_BASE_PRICE_<?= $ind ?>" value="<?echo htmlspecialchars($str_CAT_BASE_PRICE) ?>" size="15" OnBlur="ChangeSubBasePriceEx(this)">
									</td>
									<td valign="top" align="center">
										<?$db_curr = CCurrency::GetList(($by1="sort"), ($order1="asc"));?>
			...
			while ($arCatalogGroup = $dbCatalogGroups->Fetch())
			{
				...
				<tr>
					<td valign="top" align="right">
						<?echo GetMessage("C2IT_PRICE_TYPE")?> "<? echo htmlspecialchars(!empty($arCatalogGroup["NAME_LANG"]) ? $arCatalogGroup["NAME_LANG"] : $arCatalogGroup["NAME"]); ?>":
					</td>
					<td valign="top" align="left">
						<table border="0" cellspacing="1" cellpadding="3" id="SUBOTHER_PRICE_GROUP_TABLE_<?= $arCatalogGroup["ID"] ?>">
							...
							for ($i = 0; $i < $intCount; $i++)
							{
								...
								<tr id="submodel3_<?= $arCatalogGroup["ID"] ?>">
									<td valign="top" align="center">
										<input type="text" disabled readonly id="SUBCAT_QUANTITY_FROM_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" name="SUBCAT_QUANTITY_FROM_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" value="<?echo ($str_CAT_QUANTITY_FROM != 0 ? htmlspecialchars($str_CAT_QUANTITY_FROM) : "") ?>" size="3">
										<input type="hidden" name="SUBCAT_ID_<?= $arCatalogGroup["ID"] ?>[<?= $ind ?>]" value="<?= htmlspecialchars($str_CAT_ID) ?>">
									</td>
									<td valign="top" align="center">
										<input type="text" disabled readonly id="SUBCAT_QUANTITY_TO_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" name="SUBCAT_QUANTITY_TO_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" value="<?echo ($str_CAT_QUANTITY_TO != 0 ? htmlspecialchars($str_CAT_QUANTITY_TO) : "") ?>" size="3">
									...
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="SUBCAT_PRICE_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" name="SUBCAT_PRICE_<?= $arCatalogGroup["ID"] ?>_<?= $ind ?>" value="<?echo htmlspecialchars($str_CAT_PRICE) ?>" size="10" OnChange="ptSubPriceChangeEx(this)">
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="subcatalog_properties_table">
			<tr>
				...
				<td width="60%">
					...
					?>
					<input type="text" name="SUBCAT_BASE_QUANTITY" <?if ($bReadOnly) echo "disabled readonly" ?> value="<?echo htmlspecialchars($str_CAT_BASE_QUANTITY) ?>" size="30">
			...
			<tr>
				...
				<td>
					...
					?>
					<input type="checkbox" <?if ($bReadOnly) echo "disabled readonly" ?> name="SUBCAT_BASE_QUANTITY_TRACE" value="Y" <?if ($str_CAT_BASE_QUANTITY_TRACE=="Y") echo "checked"?>>
			...
			<tr>
				...
				<td>
					...
					?>
					<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> name="SUBCAT_BASE_WEIGHT" value="<?echo htmlspecialchars($str_CAT_BASE_WEIGHT) ?>" size="30">
			...
			if ($arCatalog["SUBSCRIPTION"]=="Y")
			{
				...
				<tr>
					...
					<td>
						...
						?>
						<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="SUBCAT_RECUR_SCHEME_LENGTH" name="SUBCAT_RECUR_SCHEME_LENGTH" value="<?echo htmlspecialchars($str_CAT_RECUR_SCHEME_LENGTH) ?>" size="10">
				...
				<tr>
					...
					<td>
						...
						?>
						<input id="SUBCAT_TRIAL_PRICE_ID" name="SUBCAT_TRIAL_PRICE_ID" value="<?= htmlspecialchars($str_CAT_TRIAL_PRICE_ID) ?>" size="5" type="text"><input type="button" id="SUBCAT_TRIAL_PRICE_ID_BUTTON" name="SUBCAT_TRIAL_PRICE_ID_BUTTON" value="..." onClick="window.open('cat_product_search.php?IBLOCK_ID=<?= $IBLOCK_ID ?>&amp;field_name=SUBCAT_TRIAL_PRICE_ID&amp;alt_name=subtrial_price_alt&amp;form_name='+getElementSubFormName(), '', 'scrollbars=yes,resizable=yes,width=600,height=500,top='+Math.floor((screen.height - 500)/2-14)+',left='+Math.floor((screen.width - 600)/2-5));">&nbsp;<span id="subtrial_price_alt"><?= $catProductName ?></span>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
				...
				while ($arGroup = $dbGroups->Fetch())
				{
					...
					<tr>
						...
						<td align="left"><? echo htmlspecialchars($arGroup["NAME"]); ?></td>
						<td align="center">
				...
				if ($bNoAvailGroups)
				{
					...
					<tr>
						<td colspan="3">
								...
								<?echo GetMessage("C2IT_NO_USER_GROUPS1")?> <a href="/bitrix/admin/settings.php?mid=catalog&lang=<?= LANGUAGE_ID ?>"><?echo GetMessage("C2IT_NO_USER_GROUPS2")?></a>.
		...
		if (is_array($arSKU))
		{
			...
			if ($arSKU['ID'] > 0)
			{
				...
				while ($arSKUSection = $rsSKUSections->Fetch())
					$arSKU['SECTIONS'][] = $arSKUSection['ID'];
			}
		...
		else
		{
			...
			foreach ($arDiscountList as $arProductDiscounts)
			{
				...
				?><tr>
						...
						<td align="left"<? echo $strDiscountStyle;?>><? echo htmlspecialchars($arProductDiscounts["NAME"]) ?></td>
