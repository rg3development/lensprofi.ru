		...
		if (strlen($strErrorMessage)<=0)
		{
			...
			if ($_REQUEST["ACTION"]=="EXPORT" && $bCanExec)
			{
				...
				if (strlen($strErrorMessage) <= 0)
				{
					...
					LocalRedirect("cat_export_setup.php?lang=".LANGUAGE_ID."&success_export=Y&SETUP_FILE_NAME=".urlencode($SETUP_FILE_NAME));
				}
			}
			//////////////////////////////////////////////
			// MENU
			...
			elseif ($_REQUEST["ACTION"]=="MENU" && $bCanEdit)
			{
				...
				if (strlen($strErrorMessage)<=0)
				{
					LocalRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&success_export=Y");
				}
			}
			//////////////////////////////////////////////
			// AGENT
...
foreach($arReportsList as $ReportFile=>$ReportParams):
	if(strlen($ReportParams["FILE_SETUP"])>0)
		$aAdd[] = array(
			...
			"ACTION"=>"window.location='".addslashes($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=EXPORT_SETUP"."&".bitrix_sessid_get())."';"
		);
...
<form name="agentsform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile) ?>&amp;<?= bitrix_sessid_get() ?>&amp;ACTION=AGENT&amp;PROFILE_ID=" method="post">
<table cellspacing="0">
	<tr>
...
<form name="cronform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile) ?>&amp;<?= bitrix_sessid_get() ?>&amp;ACTION=CRON&amp;PROFILE_ID=" method="post">
<table cellspacing="0">
	<tr>
		<td><?echo GetMessage("CES_RUN_INTERVAL");?></td>
		<td><input type="text" name="agent_period" value="" size="10"></td>
...
?>
<div id="agents_form_<?echo $index_form ?>" style="display:none;" class="float-form">
<form name="agentsform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile)?>&amp;<?= bitrix_sessid_get() ?>&amp;ACTION=AGENT&amp;PROFILE_ID=<?echo $ar_prof_res["ID"] ?>" method="post">
<table cellspacing="0">
	<tr>
		<td><?echo GetMessage("CES_RUN_INTERVAL");?></td>
		<td><input type="text" name="agent_period" value="" size="10"></td>
...
<div id="cron_form_<?echo $index_form ?>" style="display:none;" class="float-form">
<form name="cronform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile)?>&amp;<?= bitrix_sessid_get() ?>&amp;ACTION=CRON&amp;PROFILE_ID=<?echo $ar_prof_res["ID"] ?>" method="post">
<table cellspacing="0">
...
foreach($arReportsList as $ReportFile=>$ReportParams)
{
	...
	if(!$ar_prof_res || $ar_prof_res["DEFAULT_PROFILE"]!="Y")
	{
		...
		if ($bCanEdit)
			$row->AddField("PROFILE", '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($ReportFile)."&amp;".bitrix_sessid_get()."&amp;ACTION=EXPORT&amp;PROFILE_ID=".'" title="'.GetMessage("export_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>');
		else
		...
		if ($bCanExec)
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=EXPORT&PROFILE_ID="),
			);
		if ($bCanEdit)
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=EXPORT_SETUP"."&".bitrix_sessid_get()),
			);
		...
		if ($bCanEdit)
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=MENU&PROFILE_ID="),
			);
	...
	if($ar_prof_res)
	{
		do
		{
			...
			if ($bCanExec)
				$row->AddField("PROFILE", '<a href="'.($ar_prof_res["IN_MENU"]=="Y"? "cat_exec_exp.php":$APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($ReportFile)."&amp;ACTION=EXPORT&amp;PROFILE_ID=".$ar_prof_res["ID"]."&amp;".bitrix_sessid_get().'" title="'.GetMessage("export_setup_begin").'">'.($ar_prof_res["DEFAULT_PROFILE"]!="Y"? htmlspecialchars($ar_prof_res["NAME"]):'<i>'.GetMessage("CES_DEFAULT").'</i>').'</a>');
			else
			...
			if ($bCanExec)
				$arActions[] = array(
					...
					"TEXT"=>GetMessage("CES_RUN_EXPORT"),
					"TITLE"=>GetMessage("CES_RUN_EXPORT_DESCR"),
					"ACTION"=>$lAdmin->ActionRedirect(($ar_prof_res["IN_MENU"]=="Y"? "cat_exec_exp.php":$APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=EXPORT&PROFILE_ID=".$ar_prof_res["ID"]),
				);
			if ($bCanEdit)
			{
				if ($ar_prof_res["DEFAULT_PROFILE"] == "Y")
			...
			if($ar_prof_res["DEFAULT_PROFILE"]!="Y")
			{
				...
				if ($bCanEdit)
				{
					$arActions[] = array(
						...
						"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=EXPORT_EDIT&PROFILE_ID=".$ar_prof_res['ID']."&".bitrix_sessid_get()),
					);
					$arActions[] = array(
						"TEXT" => GetMessage("CES_COPY_PROFILE"),
						"TITLE" => GetMessage("CES_COPY_PROPFILE_DESCR"),
						"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=EXPORT_COPY&PROFILE_ID=".$ar_prof_res['ID']."&".bitrix_sessid_get()),
					);
				}
			}
			...
			if ($bCanEdit)
				$arActions[] = array(
					...
					"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=MENU&PROFILE_ID=".$ar_prof_res["ID"]),
				);
			...
			if($USER->CanDoOperation('edit_php') && ($ar_prof_res["DEFAULT_PROFILE"]!="Y" || strlen($ReportParams["FILE_SETUP"])<=0))
			{
				$arActions[] = array(
					...
					"ACTION"=>($ar_prof_res["IN_AGENT"]=="Y"? $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=AGENT&PROFILE_ID=".$ar_prof_res["ID"]) : "showAgentsForm(".$index_form.", 'agents');"),
				);
				$arActions[] = array(
					"DISABLED"=>($bWindowsHosting? true:false),
			...
			if($bCanEdit && $ar_prof_res["DEFAULT_PROFILE"]!="Y")
			{
				...
				$arActions[] = array(
					...
					"ACTION"=>"if(confirm('".GetMessage("CES_DELETE_PROFILE_CONF")."')) window.location='".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".bitrix_sessid_get()."&ACTION=DEL_PROFILE&PROFILE_ID=".$ar_prof_res["ID"]."';",
				);
			}
			$row->AddActions($arActions);
