	...
	if (strlen($strErrorMessage)<=0 && strlen($_REQUEST["ACTION"])>0)
	{
		...
		if (strlen($strErrorMessage)<=0)
		{
			...
			if ($bCanExec && $_REQUEST["ACTION"]=="IMPORT")
			{
				...
				if (!$bAllDataLoaded)
				{
					...
					<body>
						...
						<a href="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID; ?>&<?echo $urlParams ?>"><?echo GetMessage("CATI_AUTO_REFRESH_STEP");?></a><br>
						<script language="JavaScript" type="text/javascript">
						<!--
						function DoNext()
						{
							window.location="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo LANGUAGE_ID?>&<?echo $urlParams ?>&<?=bitrix_sessid_get()?>";
						}
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					LocalRedirect("cat_import_setup.php?lang=".LANGUAGE_ID."&success_import=Y&message_sess_id=".$randVal);
				}
			}
			//////////////////////////////////////////////
			// MENU
			...
			elseif ($bCanEdit && $_REQUEST["ACTION"]=="MENU")
			{
				...
				if (strlen($strErrorMessage)<=0)
				{
					LocalRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&success_import=Y");
				}
			}
			//////////////////////////////////////////////
			// AGENT
...
foreach($arReportsList as $ReportFile=>$ReportParams)
{
	...
	if(!$ar_prof_res || $ar_prof_res["DEFAULT_PROFILE"]!="Y")
	{
		...
		if ($bCanExec)
			$row->AddField("PROFILE", '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($ReportFile)."&amp;ACTION=IMPORT&amp;PROFILE_ID=".'&amp;'.bitrix_sessid_get().'" title="'.GetMessage("import_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>');
		else
		...
		if ($bCanExec)
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT&PROFILE_ID=&".bitrix_sessid_get())
			);
		if ($bCanEdit)
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT_SETUP&".bitrix_sessid_get()),
			);
		...
		if ($bCanEdit)
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=MENU&PROFILE_ID=&".bitrix_sessid_get())
			);
	...
	if($ar_prof_res)
	{
		do
		{
			...
			if ($bCanExec)
				$row->AddField("PROFILE", '<a href="'.($ar_prof_res["IN_MENU"]=="Y"? "cat_exec_imp.php":$APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($ReportFile)."&amp;ACTION=IMPORT&amp;PROFILE_ID=".$ar_prof_res["ID"].'&amp;'.bitrix_sessid_get().'" title="'.GetMessage("import_setup_begin").'">'.($ar_prof_res["DEFAULT_PROFILE"]!="Y"? htmlspecialchars($ar_prof_res["NAME"]):'<i>'.GetMessage("CES_DEFAULT").'</i>').'</a>');
			else
			...
			if ($bCanExec)
				$arActions[] = array(
					"DEFAULT"=>($ar_prof_res["DEFAULT_PROFILE"] == "Y" ? true : false),
					"TEXT"=>GetMessage("CES_RUN_IMPORT"),
					"TITLE"=>GetMessage("CES_RUN_IMPORT_DESCR"),
					"ACTION"=>$lAdmin->ActionRedirect(($ar_prof_res["IN_MENU"]=="Y"? "cat_exec_imp.php":$APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT&PROFILE_ID=".$ar_prof_res["ID"]."&".bitrix_sessid_get()),
				);
			if ($bCanEdit)
			{
			...
			if($ar_prof_res["DEFAULT_PROFILE"]!="Y")
			{
				...
				if ($bCanEdit)
				{
					$arActions[] = array(
						...
						"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT_EDIT&PROFILE_ID=".$ar_prof_res['ID']."&".bitrix_sessid_get()),
					);
					$arActions[] = array(
						"TEXT" => GetMessage("CES_COPY_PROFILE"),
						"TITLE" => GetMessage("CES_COPY_PROPFILE_DESCR"),
						"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT_COPY&PROFILE_ID=".$ar_prof_res['ID']."&".bitrix_sessid_get()),
					);
				}
			}
			...
			if ($bCanEdit)
				$arActions[] = array(
					...
					"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=MENU&PROFILE_ID=".$ar_prof_res["ID"]."&".bitrix_sessid_get()),
				);
			...
			if ($USER->CanDoOperation('edit_php') && ($ar_prof_res["DEFAULT_PROFILE"]!="Y" || strlen($ReportParams["FILE_SETUP"])<=0))
			{
				$arActions[] = array(
					...
					"ACTION"=>($ar_prof_res["IN_AGENT"]=="Y"? $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=AGENT&PROFILE_ID=".$ar_prof_res["ID"]."&".bitrix_sessid_get()) : "showAgentsForm(".$index_form.", 'agents');"),
				);
				$arActions[] = array(
					"DISABLED"=>($bWindowsHosting? true:false),
			...
			if ($bCanEdit && $ar_prof_res["DEFAULT_PROFILE"]!="Y")
			{
				...
				$arActions[] = array(
					...
					"ACTION"=>"if(confirm('".GetMessage("CES_DELETE_PROFILE_CONF")."')) window.location='".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=DEL_PROFILE&PROFILE_ID=".$ar_prof_res["ID"]."&".bitrix_sessid_get()."';",
				);
			}
			$row->AddActions($arActions);
...
foreach($arReportsList as $ReportFile=>$ReportParams):
	if(strlen($ReportParams["FILE_SETUP"])>0)
		$aAdd[] = array(
			...
			"ACTION"=>"window.location='".addslashes($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&ACTION=IMPORT_SETUP")."&".bitrix_sessid_get()."';"
		);
...
<form name="agentsform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile) ?>&amp;ACTION=AGENT&amp;PROFILE_ID=" method="post">
<?=bitrix_sessid_post();?>
<table cellspacing="0">
	<tr>
...
<form name="cronform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile) ?>&amp;ACTION=CRON&amp;PROFILE_ID=" method="post">
<?=bitrix_sessid_post();?>
<table cellspacing="0">
	<tr>
		<td><?echo GetMessage("CES_RUN_INTERVAL");?></td>
...
?>
<div id="agents_form_<?echo $index_form ?>" style="display:none;" class="float-form">
<form name="agentsform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile)?>&amp;ACTION=AGENT&amp;PROFILE_ID=<?echo $ar_prof_res["ID"] ?>" method="post">
<?=bitrix_sessid_post();?>
<table cellspacing="0">
	<tr>
		<td><?echo GetMessage("CES_RUN_INTERVAL");?></td>
...
<div id="cron_form_<?echo $index_form ?>" style="display:none;" class="float-form">
<form name="cronform_<?echo $index_form ?>" action="<?echo $APPLICATION->GetCurPage() ?>?lang=<?echo LANGUAGE_ID ?>&amp;ACT_FILE=<?echo urlencode($ReportFile)?>&amp;ACTION=CRON&amp;PROFILE_ID=<?echo $ar_prof_res["ID"] ?>" method="post">
<?=bitrix_sessid_post();?>
<table cellspacing="0">
