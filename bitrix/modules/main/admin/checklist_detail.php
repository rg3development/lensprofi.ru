...
if($_REQUEST["TEST_ID"] && $arPoints[$_REQUEST["TEST_ID"]]):?>
    ...
    function SaveStatus(_this)
    {
		...
		ShowWaitWindow();
		Dialog.hideNotify();		
		BX.ajax.post("/bitrix/admin/checklist.php","ACTION=update&STATUS="+status+"&TEST_ID="+testID+"&COMMENTS=Y"+"&custom_comment="+BX("customer_comment").value+"&perfomer_comment="+BX("performer_comment").value+"&lang=<?=LANG;?>",TestResultSimple);
	}
