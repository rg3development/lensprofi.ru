				...
				if ($lastElem['value'] instanceof CReferenceEntityField)
				{
					...
					foreach ($refEntity->GetFields() as $refField)
					{
						if (!($refField instanceof CReferenceEntityField))
						{
							...
							//$alias = $preAlias . str_replace('.', '_', $cleanValue);
							$alias = self::GetAliasByChain($refChain);
				...
				)
				{
					...
					foreach ($refEntity->GetFields() as $refField)
					{
						if (!($refField instanceof CReferenceEntityField))
						{
							...
							// алиасинг
							$cleanValue = str_replace('.*', '', $value) . '.' . $refField->GetName();
							//$alias = str_replace('.', '_', $cleanEntityValue) . '_' . $preAlias . '_' . $refField->GetName();
//							$alias = str_replace('.', '_', $cleanEntityValue)
//									. '_' . $lastElem['value'][1]->GetName()
//									. '_' . $refField->GetName();
							$alias = self::GetAliasByChain($refChain);
							$chainByAlias[$alias] = $refChain;
							$aliases[$cleanValue] = $alias;
				...
				else
				{
					...
					$alias = is_numeric($key) ? $preAlias . str_replace('.', '_', $value) : $key;
					//$alias = self::GetAliasByChain($objChain);
					$alias = is_numeric($key) ? self::GetAliasByChain($objChain) : $key;
		...
		foreach ($globChains as &$chain)
		{
			...
			foreach ($chain['value'] as &$chainElem)
			{
				if ($chainElem['value'] instanceof CReferenceEntityField)
				{
					...
					//$tableAlias = $prevAlias . '_' . strtolower($chainElem['value']->GetName());
					$tableAlias = !empty($prevAliasPrefix)
						? $prevAliasPrefix . '_' . strtolower($chainElem['value']->GetName())
						: strtolower($chainElem['value']->GetName());
				...
				)
				{
					...
					//$tableAlias = self::camel2snake($chainElem['value'][0]->GetName()) . '_' . $prevAlias;
...
//			}
	...
	protected function BuildJoin($joinTables)
	{
		...
		{
			$sql[] = sprintf('%s JOIN %s %s%s%s ON %s = %s',
				$j['type'],
				/*self::$lEsc,*/ $j['table'], /*self::$rEsc,*/
				self::$lEsc, $j['alias'], self::$rEsc,
				$j['condition'][0], $j['condition'][1]
			);
		}
	...
	{
		if (array_key_exists($key = strtoupper($name), $this->fields))
		{
	...
	{
		return array_key_exists(strtoupper($name), $this->fields);
	}
	...
	public static function GetObjectChain($initEntity, $strChain)
	{
		...
		foreach ($fieldChain as $chainElem)
		{
			...
			else
			{
				// unknown chain
				throw new Exception(sprintf(
					'Unknown field definition `%s` for %s Entity.',
					$chainElem, $prev_entity->GetName()
				), 100);
			}
		}
	...
	public static function GetAliasByChain($chain)
	{
		$alias = array();
	...
	public static function GetAliasByDefinition($str)
	{
		$alias = array();
