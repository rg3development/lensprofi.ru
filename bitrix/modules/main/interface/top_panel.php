		...
		$arPanelButtons[] = array(
			...
			"MENU"=> array(
				array(
					...
					"TITLE"=>GetMessage("button_settings"),
					"ACTION"=>"jsUtils.Redirect([], '".CUtil::addslashes(BX_ROOT."/admin/settings.php?lang=".LANG."&mid=".(defined("ADMIN_MODULE_NAME")? ADMIN_MODULE_NAME:"main").($APPLICATION->GetCurPage() <> BX_ROOT."/admin/settings.php"? "&back_url_settings=".urlencode($_SERVER["REQUEST_URI"]):""))."');",
					"ICON"=>"bx-panel-admin-button-settings-icon",
				),
				array(
...
<div id="bx-panel" class="bx-panel-admin-mode">
    ...
    <div id="bx-panel-top"><div id="bx-panel-tabs"><a href="#"<?if ($USER->IsAuthorized()):?> onclick="jsStartMenu.ShowStartMenu(this);"<?endif?> id="bx-panel-menu"><span><?echo GetMessage("admin_panel_menu")?></span></a><a href="<?=$sPubUrl?>" id="bx-panel-view-tab"><span><?echo GetMessage("admin_panel_site")?></span></a><a href="<?=BX_ROOT."/admin/index.php?lang=".LANGUAGE_ID?>" id="bx-panel-admin-tab"><span class="bx-panel-admin-tab-background" id="bx-panel-admin-tab-background"<?if($aUserOptGlobal["panel_color"]!=""):?> style="background-color: <?=htmlspecialchars($aUserOptGlobal["panel_color"])?> !important"<?endif?>><span class="bx-panel-admin-tab-left"></span><span class="bx-panel-admin-tab-center"><span class="bx-panel-menu-opacity"><?echo GetMessage("admin_panel_admin")?></span></span><span class="bx-panel-admin-tab-right"></span></span></a></div><?			
		...
		if($USER->IsAuthorized())
		{
			...
			$userInfo .= '<a href="'.htmlspecialchars($APPLICATION->GetCurPage()).'?logout=yes'.htmlspecialchars(($s=DeleteParam(array("logout"))) == ""? "":"&".$s).'" id="bx-panel-logout">'.GetMessage("admin_panel_logout").'</a>';
		...
		foreach($arPanelButtons as $item)
		{
			...
			else
			{
				...
				if(is_array($item["MENU"]) && !empty($item["MENU"]))
				{
					...
					?><a class="bx-panel-admin-button<?=($item["SELECTED"] == true ? " bx-panel-admin-button-selected": "" )?>" href="javascript:void(0);" <?=(!empty($item["LINK_PARAM"]) ? $item["LINK_PARAM"] : "")?> id="<?=$id?>" hidefocus="true" title="<?=$item["TITLE"]?>"><span class="bx-panel-admin-button-lb"></span><span class="bx-panel-admin-button-text"><?if (!empty($item["ICON"])):?><span class="bx-panel-admin-button-icon <?=$item["ICON"]?>"></span><?endif?><span class="bx-admin-opacity-text"><?=$item["TEXT"]?></span><span class="bx-panel-admin-button-arrow"></span></span><span class="bx-panel-admin-button-rb"></span></a>
				...
				{
					?><a class="bx-panel-admin-button<?=($item["SELECTED"] == true ? " bx-panel-admin-button-selected" : "" )?>" href="<?=$item["LINK"]?>" <?=(!empty($item["LINK_PARAM"]) ? $item["LINK_PARAM"] : "")?> id="<?=$id?>" hidefocus="true" title="<?=$item["TITLE"]?>"><span class="bx-panel-admin-button-lb"></span><span class="bx-panel-admin-button-text"><?if (!empty($item["ICON"])):?><span class="bx-panel-admin-button-icon <?=$item["ICON"]?>"></span><?endif?><span class="bx-admin-opacity-text"><?=$item["TEXT"]?></span></span><span class="bx-panel-admin-button-rb"></span></a><?
				}
