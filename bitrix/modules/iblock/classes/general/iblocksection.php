...
class CAllIBlockSection
{
	...
	function CheckFields(&$arFields, $ID=false)
	{
		...
		if(is_set($arFields, "IBLOCK_ID"))
		{
			...
			$r = CIBlock::GetList(array(), $arFilter);
			if(!$r->Fetch())
				$this->LAST_ERROR .= GetMessage("IBLOCK_BAD_BLOCK_ID")."<br>";
		}
		...
		if(strlen($this->LAST_ERROR)<=0)
		{
						...
						if($arthis = $rthis->Fetch())
						{
							...
							else
							{
								$strSql =
									...
									"WHERE bsto.ID=".$IBLOCK_SECTION_ID." ".
			...
			if(isset($arFields["IBLOCK_ID"]) && $arFields["IBLOCK_ID"] > 0)
				$IBLOCK_ID = intval($arFields["IBLOCK_ID"]); //for new section
			elseif(is_array($arthis))
				$IBLOCK_ID = intval($arthis["IBLOCK_ID"]); //for section with parent
			else
			{
				//for root section
				$rthis = CIBlockSection::GetList(Array(), Array("ID" => $ID, "CHECK_PERMISSIONS" => "N"));
				if($arthis = $rthis->Fetch())
					$IBLOCK_ID = intval($arthis["IBLOCK_ID"]);
				else
					$IBLOCK_ID = 0;
			}
	...
	function GetMixedList($arOrder=Array("SORT"=>"ASC"), $arFilter=Array(), $bIncCnt = false, $arSelectedFields = false)
	{
		...
		$arSectionFilter = array (
			...
			"CHECK_PERMISSIONS"	=>$arFilter["CHECK_PERMISSIONS"],
		);
