...
class CIBlockDocument
{
	function GetFieldInputControl($documentType, $arFieldType, $arFieldName, $fieldValue, $bAllowSelection = false, $publicMode = false)
	{
		...
		elseif ($arFieldType["Type"] == "user")
		{
			...
			?><input type="text" size="40" id="id_<?= $arFieldName["Field"] ?>" name="<?= $arFieldName["Field"] ?>" value="<?= htmlspecialchars($fieldValue) ?>"><input type="button" value="..." onclick="BPAShowSelector('id_<?= $arFieldName["Field"] ?>', 'user');"><?
		}
		else
		{
	...
	/**
	* ћетод по коду документа возвращает ссылку на страницу документа в административной части.
	*
	...
	*/
	...
	/**
	* ћетод возвращает свойства (пол€) документа в виде ассоциативного массива вида array(код_свойства => значение, ...). ќпределены все свойства, которые возвращает метод GetDocumentFields.
	*
	* @param string $documentId - код документа.
	...
	*/
	...
	public function AddDocumentField($documentType, $arFields)
	{
		...
		elseif ($arFields["type"] == "select")
		{
			...
			if (is_array($arFields["options"]) > 0)
			{
				$i = 10;
	...
	public function UpdateDocument($documentId, $arFields)
	{
		...
		foreach ($arKeys as $key)
		{
			...
			elseif ($arDocumentFields[$key]["Type"] == "select")
			{
				...
				$arV = array();
				$db = CIBlockProperty::GetPropertyEnum($realKey);
				while ($ar = $db->GetNext())
					$arV[$ar["XML_ID"]] = $ar["ID"];
	...
	function CanUserOperateDocument($operation, $userId, $documentId, $arParameters = array())
	{
		...
		{
			if(CIBlock::GetArrayByID($arParameters["IBlockId"], "RIGHTS_MODE") === "E")
			{
				...
				if(CIBlockElementRights::UserHasRightTo($arParameters["IBlockId"], $documentId, "element_edit"))
					return true;
				$arParameters["IBlockPermission"] = 'U';
			}
			elseif(CModule::IncludeModule('lists'))
	...
	function CanUserOperateDocumentType($operation, $userId, $documentType, $arParameters = array())
	{
		...
		if (!array_key_exists("IBlockPermission", $arParameters))
		{
			if(CIBlock::GetArrayByID($arParameters["IBlockId"], "RIGHTS_MODE") === "E")
			{
				if(!CIBlockRights::UserHasRightTo($arParameters["IBlockId"], $arParameters["IBlockId"], "element_read"))
					return false;
				if(CIBlockRights::UserHasRightTo($arParameters["IBlockId"], $arParameters["IBlockId"], "element_edit"))
					return true;
				$arParameters["IBlockPermission"] = 'U';
			}
			elseif(CModule::IncludeModule('lists'))
	...
	*/
	public function CreateDocument($parentDocumentId, $arFields)
	{
		...
		$ar = self::GetDocumentFields("iblock_".$arFields["IBLOCK_ID"]);
		...
		foreach ($arKeys as $key)
		{
			...
			if (substr($key, 0, strlen("PROPERTY_")) == "PROPERTY_")
			{
				...
				$arFieldsPropertyValues[$realKey] = is_array($arFields[$key]) ? $arFields[$key] : array($arFields[$key]);
				if ($ar[$key]["Type"] == "select")
				{
					$arV = array();
					$db = CIBlockProperty::GetPropertyEnum($key);
					while ($ar = $db->GetNext())
						$arV[$ar["XML_ID"]] = $ar["ID"];

					foreach ($arFieldsPropertyValues[$realKey] as &$value)
					{
						if (array_key_exists($value, $arV))
							$value = $arV[$value];
					}
	...
	/**
	* ћетод удал€ет указанный документ.
	*
	* @param string $documentId - код документа.
	*/
	...
	*/
	...
	// array("read" => "≈та чтение", "write" => "≈та запысь")
	...
	function GetFieldInputControlOptions($documentType, &$arFieldType, $jsFunctionName, &$value)
	{
		...
		elseif ($arFieldType["Type"] == "select")
		{
			...
			$result .= '<textarea id="WFSFormOptionsX" rows="5" cols="30">'.$str.'</textarea><br />';
			...
			$result .= '<input type="button" onclick="'.$jsFunctionName.'(WFSFormOptionsXFunction())" value="'.GetMessage("IBD_DOCUMENT_XFORMOPTIONS3").'">';
		}

		return $result;
	}

	// array("1" => "јдмины", 2 => "√ости", 3 => ..., "Author" => "јфтар")
	...
	public function SetPermissions($documentId, $workflowId, $arPermissions)
	{
		$documentId = intval($documentId);
		if ($documentId <= 0)
			throw new CBPArgumentNullException("documentId");
