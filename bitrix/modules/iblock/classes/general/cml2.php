...
/*					$rs = CCatalog::GetList(array(),array("IBLOCK_ID"=>$arIBlock["ID"]));
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		else
		{
			...
			if(array_key_exists(GetMessage("IBLOCK_XML2_FILE"), $arXMLElement))
			{
	...
	function ExportElements($PROPERTY_MAP, $SECTION_MAP, $start_time, $INTERVAL, $counter_limit = 0, $arElementFilter = false)
	{
		...
		while($arElement = $rsElements->Fetch())
		{
			...
			if(!$this->only_price)
			{
				...
				fwrite($this->fp, "\t\t\t\t<".GetMessage("IBLOCK_XML2_PICTURE").">".htmlspecialchars($this->ExportFile($arElement["DETAIL_PICTURE"]))."</".GetMessage("IBLOCK_XML2_PICTURE").">\n");
