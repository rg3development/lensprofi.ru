<?
$MESS["IBLOCK_AT_NOT_SET"] = "(не установлено)";
$MESS["IBLOCK_AT_PROP_ADD"] = "Добавить";
$MESS["IBLOCK_AT_PROP_DESC"] = "Описание значения свойства";
$MESS["IBLOCK_AT_PROP_DESC_1"] = "Описание:";
$MESS["IBLOCK_AT_PROP_NO"] = "(нет)";
$MESS["IBLOCK_AT_PROP_NA"] = "(не установлено)";
$MESS["IBLOCK_AT_OVERWRITE_TIP"] = "Назначенные права вложенных объектов будут заменены на указанные.";
$MESS["IBLOCK_AT_OVERWRITE_1"] = "Применить к подразделам и элементам";
$MESS["IBLOCK_AT_OVERWRITE_2"] = "Применить к подразделам";
$MESS["IBLOCK_AT_OVERWRITE_3"] = "Применить к элементам";
?>