...
if ((true == isset($_REQUEST['PRODUCT_NAME'])) && ('' != trim($_REQUEST['PRODUCT_NAME'])))
{
	...
	$_REQUEST['PRODUCT_NAME'] = htmlspecialchars_decode($_REQUEST['PRODUCT_NAME']);
	$_POST['PRODUCT_NAME'] = htmlspecialchars_decode($_POST['PRODUCT_NAME']);
}
...
do{ //one iteration loop
	...
	if($bTab2) $aTabs[] = array("DIV" => "sub_edit2", "TAB" => $arIBlock["SECTIONS_NAME"], "ICON"=>"iblock_element_section", "TITLE"=>htmlspecialcharsex($arIBlock["SECTIONS_NAME"]));
	$aTabs[] = array("DIV" => "sub_edit3", "TAB" => GetMessage("IBLOCK_EL_TAB_MO"), "ICON"=>"iblock_element_params", "TITLE"=>GetMessage("IBLOCK_EL_TAB_MO_TITLE"));
	if($bTab4) $aTabs[] = array("DIV" => "sub_edit4", "TAB" => GetMessage("IBLOCK_EL_TAB_WF"), "ICON"=>"iblock_element_wf", "TITLE"=>GetMessage("IBLOCK_EL_TAB_WF_TITLE"));
	if ($bTab7) $aTabs[] = array("DIV" => "sub_edit7", "TAB" => GetMessage("IBEL_E_TAB_BIZPROC"), "ICON"=>"iblock_element_wf", "TITLE"=>GetMessage("IBEL_E_TAB_BIZPROC"));
	...
	$tabControl = new CAdminForm($bCustomForm? "tabControl_sub": "form_subelement_".$IBLOCK_ID, $aTabs);
	...
	elseif ($bBizproc)
	{
		...
		{
			$canWrite = CBPDocument::CanUserOperateDocument(
				$view=="Y" ? CBPCanUserOperateOperation::ReadDocument : CBPCanUserOperateOperation::WriteDocument,
				$GLOBALS["USER"]->GetID(),
	...
	if($historyId <= 0 && $REQUEST_METHOD=="POST" && strlen($Update)>0 && $view!="Y" && (!$error) && empty($dontsave))
	{
		...
		else
		{
			...
			{
				if ($bCatalog && file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/catalog/admin/templates/subproduct_edit_validator.php")
				)
				{
				...
				if (strlen($strWarning) <= 0)
				{
					...
					{
						$arPREVIEW_PICTURE = CFile::MakeFileArray($_SERVER["DOCUMENT_ROOT"].$_REQUEST["SUB_PREVIEW_PICTURE"]);
					...
					{
						$arDETAIL_PICTURE = CFile::MakeFileArray($_SERVER["DOCUMENT_ROOT"].$_REQUEST["SUB_DETAIL_PICTURE"]);
...
$strFormAction = "/bitrix/admin/iblock_subelement_edit.php?type=".urlencode($type)."&lang=".LANG."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section);
/*if ((true == defined('BT_UT_AUTOCOMPLETE')) && (1 == BT_UT_AUTOCOMPLETE))
{
