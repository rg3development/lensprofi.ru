...
if (true == $boolSKU && true == $boolSKUFiltrable)
{
	for($i = 0, $intPropCount = count($arSKUProps); $i < $intPropCount; $i++)
	{
		if($arSKUProps[$i]["FILTRABLE"]!="Y" || $arProps[$i]["PROPERTY_TYPE"]=="F" || $arCatalog['OFFERS_PROPERTY_ID'] == $arSKUProps[$i]['ID'])
			continue;
...
if (true == $boolSKU && true == $boolSKUFiltrable)
{
	for($i = 0, $intPropCount = count($arSKUProps); $i < $intPropCount; $i++)
	{
		...
		{
			if (true == array_key_exists("AddFilterFields", $arSKUProps[$i]["PROPERTY_USER_TYPE"]))
			{
...
if ((true == $boolSKU) && (1 < sizeof($arSubQuery)))
{
...
if($lAdmin->EditAction())
{
	...
	if($bCatalog)
	{
		if($USER->CanDoOperation('catalog_price') && (isset($_POST["CATALOG_PRICE"]) || isset($_POST["CATALOG_CURRENCY"])))
		{
			...
			{
				if(!CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $elID, "element_edit_price"))
					continue;
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	foreach($arSelectedProps as $aProp)
	{
		...
		if(count($arEditHTML) > 0)
			$row->arRes['props']["PROPERTY_".$aProp['ID']] = $arEditHTML;
	}
	...
	if ($bBizproc)
	{
		...
		$arRes["CURRENT_USER_GROUPS"] = $GLOBALS["USER"]->GetUserGroupArray();
...
foreach($arRows as $f_ID => $row)
{
	...
	if(isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_edit"]))
	{
		...
		foreach($row->arRes['props'] as $prop_id => $arEditHTML)
			$row->AddEditField($prop_id, '<table id="tb'.$table_id.'" border=0 cellpadding=0 cellspacing=0><tr><td nowrap>'.implode("</td></tr><tr><td nowrap>", $arEditHTML).'</td></tr></table>');
	...
	)
	{
		...
		if($row->arRes['orig']['LOCK_STATUS'] == "red" && CWorkflow::IsAdmin())
		{
			$arActions[] = $arUnLock;
		}
		elseif(
		...
		)
		{
			...
			if($row->arRes['orig']['WF_NEW'] == "Y") // not published, under workflow
			{
				$arActions[] = array(
					"ICON" => "edit",
			...
			elseif($row->arRes["WF_STATUS_ID"] > 1) // published but changed
			{
				$arActions[] = array(
					"ICON" => "edit",
					"DEFAULT" => true,
...
{
	<?=$sTableID."_filter";?>.OnSet('<?echo CUtil::JSEscape($sTableID)?>', '<?echo CUtil::JSEscape($APPLICATION->GetCurPage().'?type='.urlencode($type).'&IBLOCK_ID='.urlencode($IBLOCK_ID).'&lang='.urlencode(LANG).'&')?>');
	return false;
}
...
function deleteFilter()
{
	...
	<?=$sTableID."_filter"?>.OnClear('<?echo CUtil::JSEscape($sTableID)?>', '<?echo CUtil::JSEscape($APPLICATION->GetCurPage().'?type='.urlencode($type).'&IBLOCK_ID='.urlencode($IBLOCK_ID).'&lang='.urlencode(LANG).'&')?>');
	return false;
}
</script><?php
$oFilter->Begin();
...
<?
$oFilter->Buttons();
?><input type="submit" name="set_filter" value="<?php echo GetMessage("admin_lib_filter_set_butt"); ?>" title="<?php echo GetMessage("admin_lib_filter_set_butt_title"); ?>" onClick="return applyFilter();">
<input type="submit" name="del_filter" value="<?php echo GetMessage("admin_lib_filter_clear_butt"); ?>" title="<?php echo GetMessage("admin_lib_filter_clear_butt_title"); ?>" onClick="deleteFilter(); return false;">
<?php
$oFilter->End();
?>
</form>
...
if(CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit"))
{
	echo
		...
		' <a href="'.htmlspecialchars('iblock_edit.php?type='.urlencode($type).'&lang='.urlencode(LANG).'&ID='.urlencode($IBLOCK_ID).'&admin=Y&return_url='.urlencode("iblock_element_admin.php?".$sThisSectionUrl)).'">',
