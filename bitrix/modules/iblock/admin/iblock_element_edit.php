...
do{ //one iteration loop
	...
	if($arIBlock)
	{
		...
		if($bBadBlock && isset($_POST["IBLOCK_SECTION"]) && is_array($_POST["IBLOCK_SECTION"]))
		{
	...
	)
	{
		...
		else
		{
			...
			if (strlen($strWarning) <= 0)
			{
				...
				if($arIBlock["RIGHTS_MODE"] === "E" && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit_rights"))
				{
...
/*	$intSKU = '';
	...
	if($arTranslit["USE_GOOGLE"] === "Y" && COption::GetOptionString("main", "translate_key_bing", "") == "")
...
if ($bOffers)
{
	...
	$boolIncludeOffers = false;
	$boolSubCatalog = (true == is_array($arSubCatalog) ? true : false);
	...
	if($boolSubWorkFlow
		|| $boolSubBizproc)
		$boolIncludeOffers = ($strSubIBlockPerm>="U");
	else
		$boolIncludeOffers = ($strSubIBlockPerm>="W");
	...
	if ((true == $boolIncludeOffers) && (true == file_exists($_SERVER["DOCUMENT_ROOT"].'/bitrix/modules/iblock/admin/templates/iblock_subelement_list.php')))
	{
