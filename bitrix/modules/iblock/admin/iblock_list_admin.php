		...
		if($TYPE=="E")
		{
			...
			elseif ($bBizproc)
			{
				$bCanWrite = call_user_func(array(ENTITY, "CanUserOperateDocument"),
					...
					$USER->GetID(),
					$ID,
					array("IBlockId" => $IBLOCK_ID, "UserGroups" => $USER->GetUserGroupArray())
				);
				if(!$bCanWrite)
				{
					$lAdmin->AddUpdateError(GetMessage("IBLIST_A_UPDERR_ACCESS", array("#ID#" => $ID)), $TYPE.$ID);
...
if(($arID = $lAdmin->GroupAction()))
{
	...
	foreach($arID as $ID)
	{
		...
		case "add_section":
			...
			{
				if($TYPE=="S" && CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $new_section, "section_section_bind"))
				{
					$obS = new CIBlockSection();
					if(!$obS->Update($ID, array("IBLOCK_SECTION_ID" => $new_section)))
						$lAdmin->AddGroupError(GetMessage("IBLIST_A_SAVE_ERROR", array("#ID#" => $ID, "#ERROR_MESSAGE#" => $obS->LAST_ERROR)), $TYPE.$ID);
				}
				elseif($TYPE=="E" && CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $new_section, "section_element_bind"))
				{
					$obE = new CIBlockElement();
		...
		case "wf_status":
			...
			break;
		case "lock":
			if($TYPE=="E" && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit"))
	...
	elseif($bWorkFlow)
	{
		...
		))
		{
			...
			elseif($arRes["WF_STATUS_ID"]>1) // already published element has unpublished changes
			{
				...
				if(CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $f_ID, "element_edit") && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $f_ID, "element_edit_any_wf_status"))
				{
					$arActions[] = array("SEPARATOR"=>true);
