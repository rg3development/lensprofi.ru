			...
			if(!empty($arResult["OFFERS_PROP_ROWS"]))
			{
				foreach($arResult["OFFERS_PROP_ROWS"] as $arProp)
				{
					...
					foreach($arProp as $propCode)
					{
						...
						{
							$url = htmlspecialchars($APPLICATION->GetCurPageParam("action=DELETE_FEATURE&op_code=".$propCode,array("op_code","action")));
							?>
							<input type="checkbox" id="<?=$propCode?>" checked="checked" onclick="location.href='<?=CUtil::JSEscape($url)?>'"><a href="<?=$url?>" rel="nofollow"><label for="<?=$propCode?>"><?=$arResult["SHOW_OFFER_PROPERTIES"][$propCode]["NAME"]?></label></a>
							<?
						}
						elseif(!empty($arResult["DELETED_OFFER_PROPERTIES"][$propCode]))
						{
							$url = htmlspecialchars($APPLICATION->GetCurPageParam("action=ADD_FEATURE&op_code=".$propCode,array("op_code","action")));
							?>
							<input type="checkbox" id="<?=$propCode?>" onclick="location.href='<?=CUtil::JSEscape($url)?>'"><a href="<?=$url?>" rel="nofollow"><label for="<?=$propCode?>" class="unchecked"><?=$arResult["DELETED_OFFER_PROPERTIES"][$propCode]["NAME"]?></label></a>
							<?
						}
						?>
						</td>
