...
function FindTopicID($tag_name, $tag_value, $topic_name="", $form_name = "form1", $tag_size = "3", $tag_maxlength="", $button_value = "...", $tag_class="typeinput", $button_class="tablebodybutton", $search_page="/bitrix/admin/forum_topics_search.php")
{
	...
	$button_class = htmlspecialchars($button_class);
	$button_value = htmlspecialchars($button_value);
	$form_name = htmlspecialchars($form_name);
	$search_page = htmlspecialchars($search_page);
	$tag_class = htmlspecialchars($tag_class);
	$tag_maxlength = htmlspecialchars($tag_maxlength);
	$tag_name = htmlspecialchars($tag_name);
	$tag_size = htmlspecialchars($tag_size);
	$tag_value = htmlspecialchars($tag_value);
	//$topic_name = htmlspecialchars($topic_name); // contains link to topics list
...
{
	var DV_".$tag_name_x.";
	DV_".$tag_name_x." = document.getElementById(\"div_".$tag_name_js."\");";
		if (strLen(trim($form_name)) <= 0)
		{
			$strReturn .= "
	if (tv".$tag_name_x."!=document.getElementById('".$tag_name_js."').value)
	{
		tv".$tag_name_x."=document.getElementById('".$tag_name_js."').value;
		if (tv".$tag_name_x."!='')
		{
			DV_".$tag_name_x.".innerHTML = '<i>".GetMessage("MAIN_WAIT")."</i>';
			document.getElementById(\"hiddenframe".$tag_name_js."\").src='/bitrix/admin/get_topics.php?ID='+tv".$tag_name_x."+'&strName=".$tag_name_js."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
		}
		else
		...
		{
			$strReturn .= "
	if (tv".$tag_name_x."!=document.".$form_name_js."['".$tag_name_js."'].value)
	{
		tv".$tag_name_x."=document.".$form_name_js."['".$tag_name_js."'].value;
		if (tv".$tag_name_x."!='')
		{
			DV_".$tag_name_x.".innerHTML = '<i>".GetMessage("MAIN_WAIT")."</i>';
			document.getElementById(\"hiddenframe".$tag_name_js."\").src='/bitrix/admin/get_topics.php?ID='+tv".$tag_name_x."+'&strName=".$tag_name_js."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
