...
<div class="gdrssitem">
	...
	<div class="gdrssitemdetail" id="z<?=$cnt.md5($item["TITLE"])?><?=htmlspecialchars(CUtil::JSEscape($rnd))?>" style="display:none">
		...
		<div class="gdrssitemdesc"><?=htmlspecialcharsEx($item["DESCRIPTION"])?> <?if($arGadgetParams["SHOW_URL"]=="Y"):?><a href="<?=htmlspecialchars($item["LINK"])?>"><?echo GetMessage("GD_RSS_READER_RSS_MORE")?></a><?endif?></div>
	</div>
</div>
<?endforeach;?>
</div>
